    ...
    <div class="mid-view">
      ...
      <div class="handle flex">
        ...
        <div class="btn-box">
          ...
          <div class="btn-bot">
            <button type="button" class="sm-btn" data-clipboard-action="copy" :data-clipboard-text="detailData.name" @click="copyLink('.sm-btn')">复制文字</button>
    ...
    <van-overlay :show="show" @click="show = false">
      <div class="wrapper" @click.stop>
        <div class="block">
          ...
          <div class="shareCode" @touchstart="gotouchstart" @touchmove="gotouchmove" @touchend="gotouchend"><img width="100%" :src="shareCode" alt="" /></div>
        </div>
      </div>
    </van-overlay>
  </div>
...
export default {
  ...
  methods: {
        ...
        if (res.code == 0) {
          ...
          this.painting = {
            ...
            views: [
              ...
              {
                type: 'qrcode',
                content: that.$root.posterUrl + res.data.id,
                css: {
                  bottom: '104px',
                  right: '30px',
                  color: '#000',
    ...
    shareData() {
      ...
      this.https.get('/share/goods', param, '&goods_id=' + this.detailData.id + '&share_user=' + this.$root.userInfo.id).then(res => {
        console.log(res);
        Toast.clear();
        if (res.code == 0) {
        } else {
    ...
    submitView(id) {
      ...
      this.https.post('/order/submit-preview', param, '&goods_info=' + attr + '&address_id=' + that.addressId + '&type=s').then(res => {
        if (res.code == 0) {
          console.log(res);
          this.showGoods = false;
