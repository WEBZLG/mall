  ...
  <div class="order-detail">
    ...
    <div class="mid-view">
      ...
      <div class="bottom flex">
        ...
        <van-button round type="info" size="small" color="#FF9900" class="pay-btn" v-if="dataList.is_pay == 1 && dataList.is_send == 0"
          @click="goHome">返回首页</van-button>
        <van-button round type="info" size="small" color="#FF9900" class="pay-btn" v-if="is_pay == 0" @click="payFor">去支付</van-button>
        <van-button round type="info" size="small" color="#FF9900" class="pay-btn" v-if="dataList.is_send == 1" @click="express">查看物流</van-button>
        <van-button round type="info" size="small" color="#FF9900" class="pay-btn" v-if="dataList.is_send == 1&&dataList.is_confirm==0" @click="successOrder">确认收货</van-button>
      </div>
    </div>
  </div>
</template>
      ...
      successOrder() {
        ...
        Dialog.confirm({
          ...
          .then(res => {
            ...
            this.https.get('/order/confirm', param, '&order_id=' + that.order_id, ).then(res => {
              console.log(res);
              Toast.clear();
              if (res.code == 0) {
                that.orderDetail(that.order_id)
              } else {
                Toast.fail(res.msg);
              }
            });
