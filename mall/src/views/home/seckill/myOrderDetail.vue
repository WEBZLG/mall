...
export default {
  ...
  methods: {
    ...
    orderDetail(order_id) {
      ...
      this.https.get('/miaosha/details', param, '&order_id='+order_id,).then(res => {
        console.log(res);
        Toast.clear();
        if (res.code == 0) {
    ...
    payFor() {
      ...
      this.https.get('/miaosha/pay-data', param, '&pay_type=WECHAT_PAY' + '&order_id_list=[' + that.order_id + ']').then(res => {
        console.log(res);
        Toast.clear();
        if (res.code == 0) {
