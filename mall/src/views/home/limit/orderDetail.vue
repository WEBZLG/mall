...
export default {
  ...
  methods: {
    ...
    orderDetail() {
      ...
      this.https.get('/group/order/detail', param, '&order_id=' + that.order_id).then(res => {
        console.log(res);
        Toast.clear();
        if (res.code == 0) {
    ...
    payFor() {
      ...
      this.https.get('/group/order/pay-data', param, '&pay_type=WECHAT_PAY' + '&order_id=' + that.order_id ).then(res => {
        console.log(res);
        Toast.clear();
        if (res.code == 0) {
